...
export class PromptEngineer {
    ...
    generatePrompt(intent, entities, context) {
...
export class EntityExtractor {
    ...
    calculateConfidence(type, value, text) {
    ...
    deduplicateEntities(entities) {
        const deduplicated = [];
        entities.forEach(entity => {
            const overlapping = deduplicated.find(existing => existing.start < entity.end && existing.end > entity.start);
            if (!overlapping) {
...
export class NLPProcessor {
    ...
    async analyze(text, contextData) {
    ...
    determineUrgency(text, sentiment) {
        const lowerText = text.toLowerCase();
